...
def test_gcam_unit_factor_matches_file(key, cfg):
    """每个指标的 unit_factor 应与 GCAM 文件中声明的单位一致。"""
    ...
    if auto_factor is not None:
        ...
        assert cfg.gcam_unit_factor != 1.0, \
            f"{key}: unknown unit '{unit_str}' but unit_factor=1.0 (add to _KNOWN_UNIT_FACTORS)"


@pytest.mark.parametrize("raw_unit,raw_value,factor,expected", [
    ("EJ",    1.0,          1_000_000, 1_000_000.0),
    ("PJ",    1000.0,       1_000,     1_000_000.0),
    ("GWh",   1000.0,       3.6,       3_600.0),
    ("Mtoe",  1.0,          41_868,    41_868.0),
    ("Gg CO₂", 1_000_000.0, 0.001,     1_000.0),
])
...
def test_read_gcam_generic_unit_auto_detection():
    """自动检测（unit_factor=默认值）应与显式 factor 一致。"""
    from compare.common.io import read_gcam_generic
    cfg = INDICATORS["tfc"]
    ...
    for y in [2015, 2050, 2100]:
        ...
        assert diff < 1.0, f"y={y}: auto vs explicit mismatch"


@pytest.mark.parametrize("key,cfg", [
    (k, v) for k, v in INDICATORS.items()
    if k in ("tfc", "electricity", "tes")
])
def test_iea_gcam_2015_magnitude_consistent(key, cfg):
    """IEA 基准值与 GCAM 2015 区域值应在同一量级（排除单位换算错误）。"""
    from compare.common.downscale import load_baseline, load_gcam

    iea = load_baseline(cfg)
    iea_total = sum(v for k, v in iea.items() if k not in ("twn",))
    if iea_total <= 0:
        return  # EDGAR-only indicators have no IEA

    gcam = load_gcam(cfg, "SSP126")
    gcam_2015 = float(gcam[gcam["Scenario"] == "SSP126"][2015].sum())

    # IEA 覆盖 ~115 国 vs GCAM 31 区域全局 — 比例应在 0.3~1.5 之间
    ratio = iea_total / gcam_2015 if gcam_2015 > 0 else 0
    assert 0.1 < ratio < 2.0, \
        f"{key}: IEA={iea_total:.1f} vs GCAM={gcam_2015:.1f}, ratio={ratio:.3f} " \
